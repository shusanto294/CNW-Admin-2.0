...
function cnw_initialize_enable_user_features() {
    ...
    $enable_user_features = isset($options['user_settings']) ? $options['user_settings'] : array();
	...
	if (isset($enable_user_features['cnw-removeab'])) {
		add_action('acf/init', 'remove_admin_bar');
	}
	
	
    // Display User Role Management Page
	if (isset($enable_user_features['cnw-role-management'])) {
	...
	if (isset($enable_user_features['cnw-email-users'])) {
