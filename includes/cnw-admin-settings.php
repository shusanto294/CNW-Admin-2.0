...
add_action('admin_init', 'cnw_initialize_enable_admin_features');
...
function cnw_initialize_enable_admin_features() {
	...
	$enable_admin_features = isset($options['general_settings']) ? $options['general_settings'] : array();

	//var_dump($enable_admin_features);
	...
	if (isset($enable_admin_features['cnw-pages'])) {
	...
	if (isset($enable_admin_features['cnw-ignoreupdates'])) {
	...
	if (isset($enable_admin_features['cnw-disable-autoupdates'])) {
		add_filter( 'automatic_updater_disabled', '__return_true' );
	}

	// If both Gutenberg disable options are selected, display an admin notice
	if (isset($enable_admin_features['cnw-disablegutenberg']) && isset($enable_admin_features['cnw-disablegutenberg-everywhere'])) {
	...
	if (isset($enable_admin_features['cnw-disable-gutenberg-fullscreen']) && isset($enable_admin_features['cnw-disablegutenberg-everywhere'])) {
		add_action('admin_notices', 'cnw_gutenberg_fulledit_notice');
	}
	// Disable Gutenberg on all Post Types, except Blog Posts
	if (isset($enable_admin_features['cnw-disablegutenberg'])) {
		add_filter( 'use_block_editor_for_post_type', 'disable_gutenberg_except_blog_posts', 10, 2 );
	}

	// Disable Gutenberg Everywhere
	if (isset($enable_admin_features['cnw-disablegutenberg-everywhere'])) {
	...
	if (isset($enable_admin_features['cnw-disable-gutenberg-fullscreen'])) {
		add_action('enqueue_block_editor_assets', 'disable_gutenberg_fullscreen_mode');
	}

	// Add duplicate button to post table
	if (isset($enable_admin_features['cnw-duplicate'])) {
	...
	// Hide ACF in Menu
	if (isset($enable_admin_features['cnw-hideacfmenu'])) {
		add_filter('acf/settings/show_admin', '__return_false');
	}

	// Display System Stats Menu
	if (isset($enable_admin_features['cnw-system-stats'])) {
		add_action('wp_dashboard_setup', 'add_system_info_dashboard_widget');
	}
	
	// Disable Site Health
	if (isset($enable_admin_features['cnw-site-health'])) {
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		...
		var imageHtml = '<div class="cnw-custom-image-container"><img src="/wp-content/plugins/cnw-admin/img/Cloud-Nine-Admin-IMG.png"></div>';
