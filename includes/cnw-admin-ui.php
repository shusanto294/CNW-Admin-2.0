...
function cnw_initialize_admin_ui_settings() {
    ...
    if (is_array($admin_ui_settings_value)) {
        ...
        if (isset($admin_ui_settings_value['cnw-login'])) {
            setup_login_screen();
        }
        if (isset($admin_ui_settings_value['cnw-support'])) {
            setup_support_widget();
        }
        if (isset($admin_ui_settings_value['cnw-removedashwidget'])) {
            remove_dashboard_widgets();
        }
        if (isset($admin_ui_settings_value['cnw-howdy'])) {
            replace_howdy_message();
        }
        if (isset($admin_ui_settings_value['cnw-removewplogo'])) {
            remove_wp_logo_from_admin_bar();
        }
        if (isset($admin_ui_settings_value['cnw-cleanup'])) {
            cleanup_dashboard();
        }
        if (isset($admin_ui_settings_value['cnw-customize-admin-logo']) && $customLogoUrl) {
            cnw_enable_custom_logo();
        }
    }
}
...
function cnw_custom_logo() {
    $options = get_option('cnw_settings');
    $customLogoUrl = isset($options['cnw_custom_logo']) ? $options['cnw_custom_logo'] : '';

    // Check if the custom logo URL is available
    if ($customLogoUrl) {
        // Output the CSS with the custom logo URL and set the size
        echo '<style>
            ul#adminmenu::before {
                content: "";
                ...
                width: 100%;
                height: 60px;
                background-image: url(' . esc_url($customLogoUrl) . ');
